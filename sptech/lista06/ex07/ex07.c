...
/*
    ...
    Objetivo do programa: Crie um programa que simula um pequeno sistema de login
    assim:
    a) Pergunte o login do usuário
    b) Pergunte a senha do usuário (curiosidade: se o type do input for
    "password", você não vê na tela o valor digitado)
    c) Ao clicar em "Entrar":
    Caso o login seja "SPTech" e a senha seja "digital", exibir um alert com a
    frase "Usuário autenticado com sucesso"
...
int main()
{
    ...
    if (strcmp(lLogin, LOGIN) == 0 && strcmp(lSenha, SENHA) == 0)
